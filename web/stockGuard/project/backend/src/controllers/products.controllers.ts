...
class ProductsController {
  ...
  static createProduct = errorHandler(async (req: Request, res: Response) => {
    ...
    if (
      !restOfBody.expiration_date ||
      !restOfBody.name ||
      !restOfBody.bars_code ||
      !restOfBody.unity_price ||
      !restOfBody.minorsale_price ||
      !restOfBody.wholesale_price
    ) {
      return ApiResponse.errorOperations(
        res,
        "Faltan campos obligatorios: name, bars_code, expiration date, unity_price, minorsale_price y wholesale_price son requeridos.",
        400,
      );
    }
    ...
    // Procesamos de forma segura el string "YYYY-MM-DD"
    const dateParsed = new Date(`${restOfBody.expiration_date}T00:00:00.000Z`);

    // Si la fecha construida no es un número válido, frenamos la ejecución con un 400
    if (isNaN(dateParsed.getTime())) {
      return ApiResponse.errorOperations(
        res,
        "El formato de la fecha de expiración es inválido. Debe ser YYYY-MM-DD",
        400,
      );
    }
  ...
  static updateProduct = errorHandler(async (req: Request, res: Response) => {
    ...
    if (String(id_product) === undefined || !id_product) {
      return ApiResponse.errorOperations(
        res,
        "se necesita identificacion del producto",
      );
    }
    ...
    const productUpdated = await productsModel.updateProduct(
      String(id_product!),
      productConverted,
    );
  ...
  static updateStockProduct = errorHandler(
    async (req: Request, res: Response) => {
      ...
      if (!Number(newStock) || isNaN(Number(newStock))) {
        return ApiResponse.errorOperations(
          res,
          "Dato no congruente con la operacion o invalido",
        );
      }
      const productUpdated = await productsModel.updateStockProduct(
        String(id_product),
        Number(newStock),
      );
      if (!productUpdated.id || productUpdated === undefined) {
        return ApiResponse.errorOperations(
          res,
          "error interno al actualizar el stock del producto",
          500,
        );
      }
