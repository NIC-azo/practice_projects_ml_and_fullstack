...
function Login() {
  ...
  const handleLogin = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await request<AuthResponseTypo>(
        "post",
        "/auth/login",
        formData,
      );
      ...
      <div className="relative w-full max-w-sm mx-4">
        <div className="rounded-2xl p-8 border">
          <div className="flex flex-col items-center mb-8">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4 bg-background-buttons/15">
              <i className="fa-solid fa-store text-background-buttons/90" />
            </div>
            <h1 className="text-4xl font-bold text-color-text-general mb-2">
              Stock Guard
            </h1>
            <p className="text-color-text-general">
              Ingresa tus credenciales para continuar
            </p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-xs font-medium mb-1.5 text-color-text-general">
                CORREO ELECTRONICO
              </label>
              <input
                ...
                onFocus={(e: React.FocusEvent<HTMLInputElement, Element>) =>
                  (e.target.style.borderColor =
                    "var(--color-color-text-general)")
                }
                onBlur={(e: React.FocusEvent<HTMLInputElement, Element>) =>
                  (e.target.style.borderColor =
                    "var(--shadow-blur-for-shadows)")
                }
              />
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 transition-opacity hover:opacity-100 opacity-50 text-color-text-button/90"
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-2.5 rounded-lg text-sm font-semibold transition-all duration-150 flex items-center justify-center gap-2 mt-2 ${
                loading
                  ? "bg-gray-600 cursor-not-allowed text-color-text-button"
                  : "bg-background-buttons cursor-pointer text-color-text-button"
              }`}
          ...
          <p className="text-xs text-center mt-6 font-mono">Stock Guard</p>
        </div>
      </div>
    </div>
  );
