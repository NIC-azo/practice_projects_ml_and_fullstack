...
class UsersController {
    static login = errorHandler(async (req: Request, res: Response) => {
        ...
        const userFound = await bcrypt.compare(String(password!), userGotten.passwordHashed);
        if (!userFound) {
        ...
        return ApiResponse.auth(res, genToken, "autenticado correctamente, dirigiendo ", userGotten.id);
    });
    /**
    ...
    static register = errorHandler(async (req: Request, res: Response) => {
        ...
        const validateUsernameDupplicated = await userModel.getUserByNickName(String(userName!));
        if (validateUsernameDupplicated){ 
            return ApiResponse.errorOperation(res, "ya existe un usuario con el mismo nombre de usuario, intentelo denuevo");
        }
        const validateEmailNotDupplicated = await userModel.getUserByemail(String(email!));
        ...
        const userRecorded = await userModel.registerUser({userName: String(userName!), 
            email: String(email!), passwordHashed: passwordHashed});
        if (!userRecorded) {
    ...
    static updateUser = errorHandler(async (req: Request, res: Response) => {
        ...
        if (String(restOfBody.userName)){
            ...
            const userFound = await userModel.getUserByNickName(String(restOfBody.userName!));
            if (userFound) {
                return ApiResponse.errorOperation(res, "ya existe un usuario con el mismo nickname, intentelo denuevo");
            }
        }
        if (String(restOfBody.email)){
            if (String(restOfBody.email).length < 0 || !String(restOfBody.email).includes("@")) {
                return ApiResponse.errorOperation(res, "credenciales incorrectas" );
            }
            const userFound = await userModel.getUserByemail(String(restOfBody.email!));
            if (userFound) {
                return ApiResponse.errorOperation(res, "credenciales incorrectas, intentelo denuevo");
            }
        }
        const userConverted: UserUpdate = {
            ...restOfBody,
            ...(restOfBody.password ?? {passwordHashed: await bcrypt.hash(String(restOfBody.password), Number(process.env["SALT_ROUNDS"]))})
        }
        const userUpdated = await userModel.updateUser(userId, userConverted);
        if (!userUpdated) {
            return ApiResponse.errorOperation(res, "error interno al actualizar usuario", 500);
        }
    ...
    static deleteUser = errorHandler(async (req: Request, res: Response) => {
        ...
        if (userDeleted === undefined) {
            return ApiResponse.errorOperation(res, "error interno al eliminar usuario")
        }
    ...
    static profileUser = errorHandler(async (req: Request, res: Response) => {
        ...
        if (!result) {
            return ApiResponse.errorOperation(res, "no se encontro al usuario o no esta identificado", 500)
        }
